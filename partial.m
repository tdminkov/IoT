...
if (mode == 0)
    ...
    %If there are more houses in pink, heat up all the houses in purple
    ...
    %Else either switch off or on the heating based on the temperature
    else
        middle_temp = (Tint+Ttilde+Theta)/2;
        houses_heating_up = 0;
        houses_in_purple = n_h - houses_in_pink;
        for i=1:n_h
            if (T0(i) >= Ttilde + Theta)
                therm(i) = 0;
            else 
                %If temperature close to new max temperature or 
                %less than half of the houses heating up, switch on heating
                if (T0(i) >= middle_temp || houses_heating_up < houses_in_purple/2)
                    houses_heating_up = houses_heating_up + 1;
                    therm(i) = 1;
                %Else let the house cool down
                else
                    therm(i) = 0;
                end
            end 
